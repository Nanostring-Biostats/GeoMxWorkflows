...
```{r checking package versions, eval = FALSE}
if(packageVersion("GeomxTools") < "2.1"){
    stop("Please install the most recent version of GeomxTools")
...
To demonstrate the effects of normalization, we graph representative boxplots 
...
We plot the results using unsupervised heirarchical clustering, displayed as a 
heatmap.
...
In addition to generating individual gene boxplots or volcano plots, we can 
...
* SpatialOmicsOverlay - [Bioconductor](https://bioconductor.org/packages/release/bioc/html/SpatialOmicsOverlay.html)
