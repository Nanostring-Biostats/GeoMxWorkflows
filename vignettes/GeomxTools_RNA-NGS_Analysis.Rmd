...
# data file type:
datadir <- system.file("inst", "extdata", "WTA_NGS_Example",
...
segments that have been profiled. We present this information in an Alluvial 
diagram.

```{r sampleFlowAlluvial, fig.width=10, fig.height=8, fig.wide = TRUE, message=FALSE, warning=FALSE}
library(dplyr)
...
factors_of_interest <- c("slide name", "class", "region", "segment")
sampleOverview <- pData(demoData)[factors_of_interest]

# Simplify the slide names
sampleOverview$`slide name` <- gsub("disease", "d",
                               gsub("normal", "n", sampleOverview$`slide name`))

# Create alluvial plot
p <- easyalluvial::alluvial_wide(sampleOverview, 
                                 max_variables = 15)

# Add hover option details to plot
pc <- parcats(p,
        marginal_histograms = FALSE,
        data_input = sampleOverview,
...
Let's re-plot the Alluvial diagram showing our current working dataset. This is 
now a dataset that no longer contains segments flagged by Segment QC or that 
have low gene detection rates.

``` {r replotAlluvial, fig.width = 10, fig.height = 8, fig.wide = TRUE, message = FALSE, warning = FALSE}

sampleOverviewFiltered <- pData(target_demoData)[factors_of_interest]

# Simplify the slide names
sampleOverviewFiltered$`slide name` <- gsub("disease", "d",
                               gsub("normal", "n", sampleOverviewFiltered$`slide name`))

# Create alluvial plot
p <- easyalluvial::alluvial_wide(sampleOverviewFiltered, 
                                 max_variables = 15)
